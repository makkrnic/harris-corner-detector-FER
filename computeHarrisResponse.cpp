...
void computeHarrisResponse(Mat &src, Mat &dest) {
    ...
    double *harrisResponse = (double *) calloc (numPixels, sizeof (double));
    ...
    for (int y = 1; y < imgSize.height-1; y++) {
        for (int x = 1; x < imgSize.width-1; x++) {
            ...
            for (int k = y - 1; k < y + 2; ++k) {
                for (int l = x - 1; l < x + 2; ++l) {
                    det += (1/9.0)*((int)Ixx[imgSize.width * k + l] * (int)Iyy[imgSize.width * k + l] - (int)Ixy[imgSize.width * k + l] * (int)Ixy[imgSize.width * k + l]);
                    trace += (1/9.0)*((int)Ixx[imgSize.width * k + l] + (int)Iyy[imgSize.width * k + l]);
                }
            }
            if (det == 0) {
//                printf ("\nDet je 0\n");
